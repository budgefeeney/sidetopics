...
def train(data, model, query, plan, updateVocab=True):
    ...
    K, Q, topicPrior, vocabPrior, wordDists, topicCov, weights, dtype, name = \
	    model.K, model.Q, model.topicPrior, model.vocabPrior, model.wordDists, model.topicCov, model.weights, model.dtype, model.name
    ...
    for itr in range(iterations):
        ...
        for d in range(D):
            ...
            # Update the in-counts (number of in-links to documents generated by each topic)
            new_in_counts += y.sum(axis=1)
            ...
            # Topics Step 2, the actual topics
            rhs[:]  = invCov.dot(U.dot(V[d, :]))
