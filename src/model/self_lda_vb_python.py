...
def _update_topics_at_d(d, data, wordCounts, linkCounts, topicMeans, topicPrior, diWordDists, diWordDistSums):
    ...
    :param data:  the word-count matrix and the link-count matrix
    :param wordCounts:  the number of words in each document
    :param linkCounts:  the number of out-links in each document
    ...
    wordIdx, z = _infer_word_topics_at_d(d, data, wordCounts, linkCounts, topicMeans, topicPrior, diWordDists, diWordDistSums)
    linkIdx, y = _infer_word_topics_at_d(d, data, wordCounts, linkCounts, topicMeans, topicPrior, diWordDists, diWordDistSums)
...
def _infer_word_topics_at_d(d, data, wordCounts, linkCounts, topicMeans, topicPrior, diWordDists, diWordDistSums):
    ...
    :param d:  the document for which we update the topic distribution
    :param data:  the object containing the word and link data
    :param wordCounts:  the length of each document
    :param linkCounts:  the number of out-links in a document
    :param topicMeans: the DxK matrix, where the d-th row contains the mean of all
                        per-token topic-assignments.
    :param topicPrior: the prior over topics
    :param diWordDists: the KxT matrix of word distributions, after the digamma funciton
...
def _infer_link_topics_at_d(d, data, wordCounts, linkCounts, topicMeans, topicPrior, diLinkDists, diLinkDistSums):
    ...
    :param d:  the document for which we update the topic distribution
    :param data:  the object containing the word and link data
    :param wordCoutns:  then number of words in each document
    :param linkCounts:  the number of out-links in each document
    :param topicMeans: the DxK matrix, where the d-th row contains the mean of all
                        per-token topic-assignments.
    :param topicPrior: the prior over topics
    :param diLinkDists: the KxT matrix of word distributions, after the digamma funciton
                        has been applied
    :param diLinkDistSums: the K-vector of the digamma of the sums of the Dirichlet
                            parameter vectors for each per-topic word-distribution
    :return: the indices of the non-zero words in document d, and the KxV matrix of
            topic assignments for each of the V non-zero words.
    '''
    linkIdx = data.links[d, :].indices
    distAtD = topicPrior \
            + (wordCounts[d] + linkCounts[d]) \
            * topicMeans[d, :]

    diLinkDists = topicPrior[np.newaxis, :] \
                + (wordCounts[linkIdx] + linkCounts[linkIdx])[:, np.newaxis] \
                * topicMeans[linkIdx, :]

    diLinkDistSums = fns.digamma(diLinkDists.sum(axis=1))
    fns.digamma(diLinkDists, out=diLinkDists)

    y  = diLinkDists[:, linkIdx]
    y -= diLinkDistSums[:, np.newaxis]
    y += fns.digamma(distAtD)
